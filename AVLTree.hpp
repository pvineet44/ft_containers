...
namespace ft
{
    ...
    class AVLTree
    {
		...
		public:
				...
				explicit AVLTree(const compare& cmp = compare()):_cmp(cmp), _root(NULL), _len(0)
				{
				}

				template <typename InputIt>
				AVLTree(InputIt first, InputIt last, const compare& cmp = compare()): _cmp(cmp), _root(NULL), _len(0)
				{
					insert(first, last);
				}
				...
				AVLTree<E, Compare, AllowDuplicate> &operator=(const AVLTree<E, Compare, AllowDuplicate>& other)
				{
					deep_free(_root);
